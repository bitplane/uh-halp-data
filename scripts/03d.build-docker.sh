...
base_image_hash=$(docker inspect --format='{{.Id}}' uh-halp-data-binaries:ubuntu-base)

# Split the filtered packages file into batches
batch_size=50
split -d -l $batch_size data/03b.limited-packages data/03d.packages_
...
for batch_file in data/03d.packages_*; do
    ...
    docker build \
        --build-arg BASE_IMAGE=$base_image_hash \
    ...
    # Update the base image hash for the next iteration
    base_image_hash=$(docker inspect --format='{{.Id}}' uh-halp-data-binaries:ubuntu-$batch_tag)
done

# Final image tag
docker tag $base_image_hash uh-halp-data-binaries:ubuntu-final
